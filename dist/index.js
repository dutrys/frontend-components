...
const DateInput = ({ onChange, value, className, toggleClassName, required, disabled, size, placeholder, from, to, ...rest }) => {
    ...
    return (jsx(FocusTrap, { className: "grow", features: isOpen ? FocusTrapFeatures.FocusLock : FocusTrapFeatures.None, children: jsx(Popover, { showOnClick: true, showOnFocus: true, showOnHover: false, onShow: (open) => setIsOpen(open), title: (ref, popoverProps) => (jsxs("div", { ref: ref, ...rest, ...popoverProps, className: cx("input input-bordered", {
                    ...
                    ["w-full"]: !className?.includes("w-"),
                            ...
                            e.preventDefault();
                            onChange(null);
                        }, children: jsx(XMarkIcon, { className: "size-4" }) }))] })), children: (close) => (jsx(DayPicker, { className: `react-day-picker bg-transparent border-none text-white ${styles$1.dayPicker}`, captionLayout: "dropdown", mode: "single", locale: params.locale === "lt" ? lt : enGB, showOutsideDays: true, disabled: matcher, weekStartsOn: 1, selected: value || undefined, defaultMonth: value || undefined, onSelect: (day) => {
                    onChange(day || null);
                    close();
                } })) }) }));
};
...
const SidebarMenu = ({ menu, active, expanded, }) => (jsx("ul", { className: cx("menu w-full"), children: menu.map((item, i) => {
        if (item.items && !expanded) {
            return (jsx(Popover, { backgroundColor: "bg-nav-popover/95", placement: "right-start", arrowSize: { width: 10, height: 5 }, title: (ref, props, isOpen) => (jsx("div", { ref: ref, children: jsx(Item, { disableTooltip: true, item: item, active: active(item) || (Array.isArray(item.items) && item.items.some((i) => active(i))), forceHover: isOpen }) })), children: (close) => (jsx("div", { "data-theme": "dim", className: "bg-transparent", children: Array.isArray(item.items) ? (jsxs(Fragment, { children: [jsx("div", { className: "text-xs text-center pt-2 pb-2 text-white", children: item.name }), jsx("ul", { className: "menu menu-sm px-1 pt-0 pb-0", children: item.items?.map((sub, i) => (jsx("li", { children: jsxs(Link, { href: sub.href, onClick: close, className: "text-white", children: [jsx(sub.icon, { className: "size-4" }), sub.name] }) }, i))) })] })) : (item.items()) })) }, `${item.name}-${i}`));
        }
