...
const BulkDropDownActions = ({ bulkActions, disabled, }) => bulkActions.map((bulkAction, i) => (jsx("li", { children: jsx("button", { type: "button", disabled: disabled, onClick: () => bulkAction.onSelect(), children: bulkAction.children }) }, i)));
...
const NoCountPagination = ({ nextPageAvailable }) => {
    const searchParams = useSearchParams();
    const t = useTranslations();
    const pageString = searchParams.get("page");
    const page = Math.max(Number(pageString), 1);
    return (jsxs("div", { className: "my-4 gap-2", children: [jsxs(Link, { className: cx("btn btn-sm mr-2 w-30", { "btn-disabled": page <= 1 }), href: setPartialParams({ page: page <= 2 ? "" : `${page - 1}` }, searchParams), prefetch: false, children: [jsx(ChevronLeftIcon, { className: "size-4" }), " ", t("pagination.previous")] }), jsxs(Link, { href: setPartialParams(nextPageAvailable ? { page: `${page + 1}` } : {}, searchParams), className: cx("btn btn-sm w-30", { "btn-disabled": !nextPageAvailable }), prefetch: false, children: [t("pagination.next"), " ", jsx(ChevronRightIcon, { className: "size-4" })] })] }));
};
...
const ReorderHandle = ({ dragControls }) => (jsx(motion.div, { whileTap: { scale: 0.95 }, onPointerDown: (e) => {
        e.preventDefault();
...
class LocalStorage {
    getConfig(title, columns) {
        throw new Error("Method not implemented.");
    }
    setConfig(title, configs) {
        throw new Error("Method not implemented.");
    }
}
...
const DateInput = ({ onChange, value, className, toggleClassName, required, disabled, size, placeholder, matcher, modifiers, ...rest }) => {
        ...
        if (!value) {
            ...
            queryFn({ "filter.id": [`${value}`] }).then((pager) => {
                if (pager.data.length !== 1 || (pager.data[0] && optionValue(pager.data[0])?.toString() !== `${value}`)) {
                    ...
                    if (item) {
                        console.error(`Found model #${optionValue(item)}. ${optionLabel(item)}, but pagination filtering does not work in your backend api`);
                        setValueModel(item);
...
const DateTimeFormField = ({ label, desc, control, name, disabled, error, className, fieldSetClassName, useDate, ...props }) => (jsxs("div", { className: fieldSetClassName, children: [jsxs("label", { className: "floating-label", children: [jsx(Controller, { control: control, name: name, rules: { required: props.required === true }, disabled: disabled, render: ({ field }) => (jsx(DateTimePicker, { ...props, className: cx(className, { "input-error": error }), placeholder: props.required ? `${label}*` : label, disabled: field.disabled, value: field.value ? (useDate ? field.value : stringToDate(field.value)) || null : null, onChange: (value) => {
                            if (useDate) {
                                field.onChange(value);
                            }
                            else {
...
const Title = ({ children, outerHeight, truncate, noOverlap, paddingLeft = "pl-4", noBackground, }) => (jsxs(Fragment, { children: [!noBackground && (jsx("div", { className: cx("absolute bg-base-100/50 backdrop-blur-xs z-500 w-full left-0 top-0", { "h-16": !outerHeight }, outerHeight) })), jsx("div", { className: cx("h-16 hidden sm:flex absolute z-501 flex items-center font-bold ml-[var(--top-left-bar)] w-[calc(100vw-var(--sidebar-width)-var(--top-right-bar)-var(--top-left-bar))]", { truncate }, paddingLeft), children: children }), noOverlap && jsx("div", { className: cx({ "h-16": !outerHeight }, outerHeight) })] }));

const limits = [10, 20, 50, 100];
const isActionColumn = (column) => typeof column === "object" && column.type === "actions";
const isFunctionColumn = (column) => typeof column === "object" && typeof column.body === "function";
const SearchField = () => {
    const t = useTranslations();
    const params = useParams();
    const searchParams = useSearchParams();
...
const PaginatedTable = ({ pagination, title, titleAbove, sortEnum, extraHeading, columns, caption, isSearchable = false, searchableShortcuts = [], addNew, bulkActions, addNewText, displayFilters, displayConfig, renderGridItem, rowClickHref, defaultDisplayAs = "list", toolbarClassName, }) => {
    const router = useRouter();
    const params = useParams();
    const searchParams = useSearchParams();
    const t = useTranslations();
    ...
    const { data: paginationConfigs, refetch: refetchPaginationConfigs } = useQuery({
        ...
        queryFn: async () => (await store?.getConfig(displayConfig.name, columns)) ?? { columns: {}, loadByDefault: "default" },
        initialData: displayConfig?.stored || {},
        refetchOnWindowFocus: false,
        refetchOnReconnect: false,
    });
    const columnsConfigs = paginationConfigs?.columns ?? {};
                                            ...
                                            if (displayConfig) {
                                                void store?.setConfig(displayConfig.name, { displayAs: "list" });
                                                    ...
                                                    [styles$4.rowHover]: hover === model.id,
                                                            ...
                                                            }, checked: selected.includes(model.id) }) })), columnsConfigs[configName].map((item, i) => {
                                                        const column = columns.find((c) => isActionColumn(c) ? "action" === item.name : c.name === item.name);
                                                        if (!column || !item.enabled) {
                                                            return null;
                                                        }
                                                    ...
                                                    })] }, o))) })] })) }), jsx("div", { className: "absolute box-content left-0 bottom-0 w-full h-16 z-1 border-t border-t-base-content/5", children: jsx("div", { className: "bg-base-100", children: jsxs("div", { className: "flex justify-center items-center", children: [!pagination.meta.withoutCount && (jsx("div", { className: `shrink-1 w-60 text-xs pl-4 ${pagination.meta.totalPages > 1 ? "hidden md:block" : ""}`, children: t("pagination.showing", {
                                                ...
                                                }) })), jsx("div", { className: "grow text-center h-16", children: pagination.meta.withoutCount ? (jsx(NoCountPagination, { nextPageAvailable: pagination.data.length === pagination.meta.itemsPerPage })) : (pagination.meta.totalPages > 1 && jsx(Pagination, { visiblePages: 5, page: pagination.meta })) }), pagination.meta.totalPages > 1 && (jsxs("div", { className: "shrink-1 w-60 hidden md:block text-xs text-right pr-4", children: [jsx("span", { className: "text-gray-400", children: t("pagination.itemsPerPage") }), " ", limits.map((l) => (jsx(LimitLink, { isActive: pagination.meta.itemsPerPage === l, text: l.toString(), href: setPartialParams({ page: "1", limit: `${l}` }, searchParams) }, l)))] }))] }) }) })] }))] })] }));
};
const PAGINATED_IGNORE_ROW_CLICK = "action-cell";
const LimitLink = ({ href, text, isActive }) => (jsx(Link, { prefetch: false, className: `text-gray-400 hover:text-primary-600 mr-1 ${isActive ? "font-bold text-primary-600" : ""}`, href: href, children: text }));
...
const FilterLink = ({ children, className, params, href, }) => {
    const t = useTranslations();
    const searchParams = useSearchParams();
    ...
    return (jsxs(Fragment, { children: [href ? jsx(TableLink, { href: href, children: children }) : children, "\u00A0", jsx(TableLink, { "data-tooltip-id": TOOLTIP_GLOBAL_ID, "data-tooltip-content": isFiltering ? t("general.filter") : t("general.clearFilter"), href: `${pathname}${setPartialParams(p, searchParams)}`, children: isFiltering ? (jsx(FunnelIcon, { className: cx(className, "inline size-5 text-primary") })) : (jsx(FunnelIcon$1, { className: cx(className, "inline size-5 text-primary") })) })] }));
};
const TruncateText = ({ text, length }) => (jsx("div", { "data-tooltip-id": TOOLTIP_GLOBAL_ID, "data-tooltip-content": text, className: "text-left text-ellipsis overflow-hidden", style: { width: length }, children: text }));
...
const ParallelDialogButtons = ({ children }) => jsx("div", { className: "p-4 -mx-4 mt-4 -mb-4 bg-base-200 rounded-b-field flex gap-4 justify-end", children: children });
...
const FormatErrors = ({ errors, translationId }) => {
