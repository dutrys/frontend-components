...
export function DateTimePicker({ value, onChange, allowEmpty, disabled, required, from, to, placeholder, inputClassName = "input input-bordered", toggleClassName = "", }) {
    ...
    return (<label className={`w-full ${inputClassName}`}>
      <Popover title={(ref, props) => (<input required={required} value={dateString} className="grow" disabled={disabled} ref={ref} placeholder={placeholder} onChange={(e) => {
        ...
        }} showOnClick showOnFocus showOnHover={false} popoverWidth="">
        {(close) => (<>
            <div className="flex">
              <DayPicker className={`react-day-picker bg-transparent border-none text-white ${styles.dayPicker}`} captionLayout="dropdown" mode="single" locale={params.locale === "lt" ? lt : enGB} showOutsideDays weekStartsOn={1} disabled={matcher} selected={valueTemp || undefined} defaultMonth={valueTemp || new Date()} onSelect={(day) => {
              ...
              <div>
                ...
                <div className="flex">
                  <ul className="h-53 mx-1 overflow-y-scroll" ref={hoursRef}>
                    {new Array(24).fill(0).map((_, hour) => (<li data-hour={hour} key={hour}>
                        {(from && valueTemp && isSameDay(from, valueTemp) && from.getHours() > hour) ||
                    (to && valueTemp && isSameDay(to, valueTemp) && to.getHours() < hour) ? (<></>) : (<button className={`text-white text-xs rounded-sm px-4 p-2 cursor-pointer hover:text-black ${valueTemp?.getHours() === hour ? "bg-primary-500" : "hover:bg-white"}`} onClick={() => {
                  ...
                  <ul className="h-53 mx-1 overflow-y-scroll" ref={minutesRef}>
                    {new Array(60).fill(0).map((_, minute) => (<li data-minute={minute} key={minute}>
                        {(from && valueTemp && isSameHour(from, valueTemp) && from.getMinutes() > minute) ||
                    (to && valueTemp && isSameHour(to, valueTemp) && to.getMinutes() < minute) ? (<></>) : (<button onClick={() => {
                        const date = new Date(valueTemp || new Date());
            ...
            <div className="text-center pb-2">
              <button className="btn btn-xs mx-auto w-30" type="button" onClick={() => {
                onChange(valueTemp);
                close();
            }}>
                {t("datePicker.ok")}
